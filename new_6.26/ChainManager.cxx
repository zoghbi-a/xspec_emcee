...
void ChainManager::recalc ()
{
  ...
  std::unique_ptr<double[]> pRv1(new double[nPar]);
...
void ChainManager::calcStat (size_t iPar, const string& modName)
{
  ...
  vector<RealArray> subMeans(nChains);
  ...
  for (size_t i=0; i<nChains; i++) {
    geweke[i] = (subMeans[i][0]-subMeans[i][1])/sqrt(variances[i]);
  }
