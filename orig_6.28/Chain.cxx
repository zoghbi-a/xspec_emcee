			      ...
			      std::vector<Real>& walkerStatisticValue)
{
  ...
  //    and hard limit information.
  const size_t nPars = fit->variableParameters().size();
  RealArray maxValues(nPars);
  RealArray minValues(nPars);
  RealArray paramDeltas(nPars);
  Real maxFactor = 20.0;
  if (!useCovar)
  {
     // Find the maximum randomization scaling factor that won't push any
     //  parameters beyond their hard limits.
     if (originalParamValues.size() != nPars)
        throw RedAlert("Array size mismatch in Chain::initializeWalkers");
     maxValues = fit->variableParameterValues('h');
     minValues = fit->variableParameterValues('l');
     paramDeltas = fit->variableParameterValues('d');
     for (size_t i=0; i<nPars; ++i)
     {
        Real parFactor = 2.0*(maxValues[i]-originalParamValues[i])/paramDeltas[i];
        if (parFactor < maxFactor)
           maxFactor = parFactor;
        parFactor = 2.0*(originalParamValues[i]-minValues[i])/paramDeltas[i];
        if (parFactor < maxFactor)
           maxFactor = parFactor;
     }
  }
  ...
  for (size_t i=0; i<m_walkers; i++) {
    ...
    if ( useCovar ) {
      ...
      Numerics::UniformRand(randNumber);
      walkerParamValues[i] =  originalParamValues + paramDeltas * 
                (randNumber-0.5) * maxFactor;
